            ...
            <div class="modal-body">
                <form class="form-horizontal">
                    <div class="form-group">
                        <label class="col-sm-2 control-label">商品名</label>
                        <div class="col-sm-10">
                            <input type="text" name="proName" class="form-control" id="proName_add_input"
                                   placeholder="中文开头或英文(3-16)">
                            <span class="help-block"></span>
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label">货号</label>
                        <div class="col-sm-10">
                            <input type="text" name="proNo" class="form-control" id="proNo_add_input"
                                   placeholder="6-10个字母数字的组合">
                            <span class="help-block"></span>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">单价</label>
                        <div class="col-sm-10">
                            <input type="text" name="proPrice" class="form-control" id="proPrice_add_input"
                                   placeholder="10">
                            <span class="help-block"></span>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">库存</label>
                        <div class="col-sm-10">
                            <input type="text" name="proNumber" class="form-control" id="proNumber_add_input"
                                   placeholder="100">
                            <span class="help-block"></span>
                        </div>
                    </div>
...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-4 col-md-offset-8">
            <button class="btn btn-primary btn-lg" id="pro_add_modal_btn"><span class="glyphicon glyphicon-plus"></span>新增
            </button>
            <button class="btn btn-danger btn-lg" id="pro_delete_all_btn"><span
                    class="glyphicon glyphicon-remove"></span>删除
            </button>
            <button class="btn btn-danger btn-lg" id="logout_btn" onclick="submitLogout()"><span
                    class="glyphicon glyphicon-log-out"></span>退出
            </button>

            <!--TODO-->
            <div>
                &nbsp;商品名：&nbsp;<input type="text" id="proName" size="20"
                                       onkeydown="if(event.keyCode===13) searchProName($('#proName').val())"/> <a
                    href="javascript:searchProName($('#proName').val())" iconCls="icon-search" plain="true">搜索</a>
            </div>
...
<script type="text/javascript">
    ...
    function checkSession() {
        var strSession = "<%=session.getAttribute("currentUser")%>";
        if (strSession === "null") {
            alert('请先登录');
    ...
    function build_pros_table(result) {
        ...
        $.each(pros, function (index, item) {
            ...
            //TODO: this is where cause all search functionality，but why OK while list？
//				var cateNameTd = $("<td></td>").append(item.category.cateName);
            var cateNameTd = $("<td></td>").append(item.cateName);
    ...
    //TODO：搜索展示功能
    function searchProName(str) {
        $(function () {
            //符合搜索结果过多时，返回到首页
            to_search_page(1, str);
        });
    }

    //TODO: 分页信息
    function to_search_page(pn, str) {
        $.ajax({
            url: "${APP_PATH}/search",
            data: "pn=" + pn + "&proName=" + str,
            type: "GET",
            success: function (result) {
                //console.log(result);
                //1、解析并显示商品数据
    ...
    //TODO:载入时隐藏 div 元素，在 body onLoad 中调用
    function HideDiv() {
        $("#container").hide();
    }
